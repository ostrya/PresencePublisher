...
public class BeaconCategorySupport extends AbstractDynamicPreferenceCategorySupport {
    ...
    public BeaconCategorySupport(AbstractConfigurationFragment fragment, @Nullable ActivityResultLauncher<String> intentLauncher) {
        super(fragment, R.string.category_beacon_regions, BEACON_LIST, BEACON_CONTENT_PREFIX, (c, p, f) -> createAdderEntry(c, f, intentLauncher), BeaconCategorySupport::createEntry);
    }

    public void clickAdd() {
        @Nullable Preference adderPreference = getPreferenceManager().findPreference(AddBeaconChoicePreferenceDummy.class.getCanonicalName());
        if (adderPreference != null) {
            adderPreference.getOnPreferenceClickListener().onPreferenceClick(adderPreference);
        }
    }

    private static Preference createAdderEntry(Context context, Fragment fragment, @Nullable ActivityResultLauncher<String> intentLauncher) {
    ...
    private static Preference createEntry(Context context, String key, String title, SharedPreferences preferences, Fragment fragment) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            return new BeaconPreference(context, key, title, fragment);
        } else {
            HyperLog.w(TAG, "Should never happen: tried to create beacon entries on incompatible devices!");
            return new StringDummy(context, R.string.no_bluetooth_explanation);
        }
    }
}
